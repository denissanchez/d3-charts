async function drawLineChart() {
  ...
  let dataset = await d3.json("./assets/my_weather_data.json")
  ...
  function onMouseMove(event) {
    ...
    const closesDataPoint = dataset[closestIndex];
    const closestXValue = xAccessor(closesDataPoint);
    const closestYValue = yAccessor(closesDataPoint);

    const formatDate = d3.timeFormat("%B %A %-d, %Y");

    tooltip.select("#date")
      .text(d => formatDate(closestXValue));

    const formatTemperature = d => `${d3.format(".1f")(d)} °F`;

    tooltip.select("#temperature")
      .html(d => formatTemperature(closestYValue));

    const xTooltip = xScale(closestXValue) + dimensions.margin.left;
    const yTooltip = yScale(closestYValue) + dimensions.margin.top;
